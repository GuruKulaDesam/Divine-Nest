<script>
  ...
  function navigateTo(path) {
    ...
    if ($isNavigating) {
      console.log("Navigation already in progress, ignoring bottom nav click");
      return;
    }
...
<div class="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 z-50 md:hidden">
  <div class="flex items-center justify-around px-2 py-2">
    {#each bottomNavItems as item}
      <button class="flex flex-col items-center justify-center p-2 rounded-lg transition-all duration-200 min-w-0 flex-1 {$isNavigating ? 'pointer-events-none opacity-60' : ''} {isActive(item) ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100'}" on:click={() => navigateTo(item.path)} aria-label={item.label}>
