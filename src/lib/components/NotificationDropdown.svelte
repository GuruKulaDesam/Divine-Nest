<script>
  ...
  import { notifications, getUnreadNotificationsCount, markNotificationAsRead, markAllNotificationsAsRead } from "../data/notifications.js";
...
<div bind:this={dropdownElement} class="absolute right-0 mt-2 w-80 bg-base-100 rounded-lg shadow-lg border border-base-300 z-50 max-h-96 overflow-hidden">
  ...
  <div class="flex items-center justify-between p-4 border-b border-base-300">
    ...
    <div class="flex items-center space-x-2">
      {#if unreadCount > 0}
        <button class="text-sm text-primary hover:text-primary-focus transition-colors focus:outline-none focus:ring-2 focus:ring-primary" on:click={handleMarkAllAsRead} aria-label="Mark all notifications as read"> Mark all as read </button>
      {/if}
      <button class="p-1 text-base-content/60 hover:text-base-content hover:bg-base-200 rounded transition-colors focus:outline-none focus:ring-2 focus:ring-primary" on:click={onClose} aria-label="Close notifications">
  ...
  <div class="dropdown-scroll">
    ...
    {:else}
      {#each notifications as notification (notification.id)}
        <button class="w-full p-4 border-b border-base-200 hover:bg-base-50 cursor-pointer transition-colors focus:outline-none focus:ring-2 focus:ring-primary {!notification.read ? 'bg-primary/5' : ''}" on:click={() => handleNotificationClick(notification)} use:motionHover aria-label="Notification: {notification.title}">
