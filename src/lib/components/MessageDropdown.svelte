<script>
  ...
  import { messages, getUnreadMessagesCount, markMessageAsRead, markAllMessagesAsRead } from "../data/notifications.js";
...
<div bind:this={dropdownElement} class="absolute right-0 mt-2 w-80 bg-base-100 rounded-lg shadow-lg border border-base-300 z-50 max-h-96 overflow-hidden">
  <!-- Header -->
  <div class="flex items-center justify-between p-4 border-b border-base-300">
    <h3 class="text-lg font-semibold text-base-content">Messages</h3>
    <div class="flex items-center space-x-2">
      {#if unreadCount > 0}
        <button class="text-sm text-primary hover:text-primary-focus transition-colors focus:outline-none focus:ring-2 focus:ring-primary" on:click={handleMarkAllAsRead} aria-label="Mark all messages as read"> Mark all as read </button>
      {/if}
      <button class="p-1 text-base-content/60 hover:text-base-content hover:bg-base-200 rounded transition-colors focus:outline-none focus:ring-2 focus:ring-primary" on:click={onClose} aria-label="Close messages">
  ...
  <div class="dropdown-scroll">
    ...
    {:else}
      {#each messages as message (message.id)}
        <button class="w-full p-4 border-b border-base-200 hover:bg-base-50 cursor-pointer transition-colors focus:outline-none focus:ring-2 focus:ring-primary {!message.read ? 'bg-primary/5' : ''}" on:click={() => handleMessageClick(message)} use:motionHover aria-label="Message from {message.sender.name}: {message.subject}">
  ...
  {#if messages.length > 0}
    <div class="p-3 border-t border-base-300 bg-base-50">
      <button
        class="w-full text-sm text-primary hover:text-primary-focus font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-primary"
        on:click={() => {
