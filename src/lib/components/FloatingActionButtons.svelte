...
<div class="fixed bottom-6 right-6 z-50 flex flex-col items-end space-y-3">
  ...
  {#if isExpanded}
    <div class="flex flex-col space-y-3 animate-fade-in">
      <!-- AI Assistant Button -->
      <button class="w-12 h-12 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-110 flex items-center justify-center group" on:click={handleAIAssistant} on:mouseenter={handleHover} title="AI Assistant">
        <Icon icon="heroicons:sparkles" class="w-6 h-6 group-hover:animate-pulse" />
      </button>

      <!-- WhatsApp Share Button -->
      <button class="w-12 h-12 rounded-full bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-110 flex items-center justify-center group" on:click={handleWhatsAppShare} on:mouseenter={handleHover} title="Share via WhatsApp">
        <Icon icon="logos:whatsapp-icon" class="w-6 h-6" />
      </button>

      <!-- Email Share Button -->
      <button class="w-12 h-12 rounded-full bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-110 flex items-center justify-center group" on:click={handleEmailShare} on:mouseenter={handleHover} title="Share via Email">
  ...
  <button class="w-10 h-10 rounded-full bg-gray-800/80 hover:bg-gray-700/80 text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-110 flex items-center justify-center backdrop-blur-sm" on:click={toggleExpanded} on:mouseenter={handleHover} title={isExpanded ? "Collapse" : "Expand"}>
