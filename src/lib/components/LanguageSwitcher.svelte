...
<div class="relative language-switcher">
  <button class="p-2 text-base-content/60 hover:text-base-content hover:bg-base-200 rounded-lg transition-colors duration-200 flex items-center space-x-2 focus:outline-none focus:ring-2 focus:ring-primary" on:click={toggleDropdown} use:motionHover aria-label="Select language" aria-expanded={isOpen}>
  ...
  {#if isOpen}
    <div bind:this={dropdownElement} class="absolute right-0 mt-2 w-48 bg-base-100 rounded-lg shadow-lg border border-base-300 py-1 z-50">
      {#each Object.entries(languages) as [code, lang]}
        <button class="w-full flex items-center px-4 py-2 text-sm hover:bg-base-200 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-primary {$currentLanguage === code ? 'bg-primary/10 text-primary' : 'text-base-content'}" on:click={() => selectLanguage(code)}>
