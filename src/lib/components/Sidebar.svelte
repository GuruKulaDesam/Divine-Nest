<script>
  ...
  $: translatedMenuItems = [
    ...
    { path: "/family-calendar", icon: "heroicons:calendar-days", name: "Family Calendar" },
    { path: "/family-calendar-modern", icon: "heroicons:calendar-days", name: "Calendar (Modern)" },
    { path: "/family-todo", icon: "heroicons:check-circle", name: "Family To-Do" },
    { path: "/family-map", icon: "heroicons:map-pin", name: "Family Map" },
    { path: "/family-notes", icon: "heroicons:document-text", name: "Family Notes" },
    { path: "/family-notes-modern", icon: "heroicons:document-text", name: "Notes (Modern)" },
    { path: "/family-notifications", icon: "heroicons:bell", name: "Family Notifications" },
    { path: "/reminders", icon: "heroicons:bell-alert", name: "Smart Reminders" },
  ];
...
<div class="fixed inset-y-0 left-0 z-50 w-64 bg-base-100 shadow-lg transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 {isOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col h-full">
  ...
  <div class="flex items-center justify-center h-16 px-6 border-b border-base-300 flex-shrink-0">
    <div class="flex items-center space-x-3">
      <div class="relative">
        <Icon icon="heroicons:home" class="w-6 h-6 text-blue-500 animate-pulse" />
        <div class="absolute -top-1 -right-1 w-3 h-3 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full animate-bounce"></div>
      </div>
      <div class="flex flex-col">
        <span class="text-lg font-bold bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent animate-pulse"> Home </span>
        <span class="text-sm font-semibold bg-gradient-to-r from-green-500 via-blue-500 to-purple-500 bg-clip-text text-transparent -mt-1"> Manager </span>
      </div>
    </div>
  </div>
  ...
  <nav class="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
    {#each translatedMenuItems as item, index}
      <a bind:this={menuItemsElements[index]} href={item.path} class="w-full flex items-center px-4 py-3 text-sm font-medium rounded-lg transition-colors duration-200 {$currentRoute === item.path ? 'bg-blue-500/10 text-blue-600 border border-blue-500/20' : 'text-base-content/70 hover:bg-base-200 hover:text-base-content'}" on:click|preventDefault={() => handleMenuClick(item.path)} use:motionInView={{ animation: "fadeInLeft", delay: index * 0.1 }}>
    ...
    <div class="mt-6">
      ...
      <div class="submenu">
        <button class="w-full flex items-center justify-between px-4 py-3 text-sm font-medium rounded-lg transition-colors duration-200 {isSubmenuActive(authSubmenu.items) ? 'bg-blue-500/10 text-blue-600 border border-blue-500/20' : 'text-base-content/70 hover:bg-base-200 hover:text-base-content'}" on:click={() => toggleSubmenu("auth")}>
          <div class="flex items-center">
            <Icon icon={authSubmenu.icon} class="w-5 h-5 mr-3" />
            {authSubmenu.title}
          </div>
          <Icon icon={expandedMenus.auth ? "heroicons:chevron-down" : "heroicons:chevron-right"} class="w-4 h-4 transition-transform" />
        </button>

        {#if expandedMenus.auth}
          <div class="ml-4 pl-4 border-l border-base-300 mt-1 space-y-1">
            {#each authSubmenu.items as subitem}
              <a href={subitem.path} class="w-full flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-200 {$currentRoute === subitem.path ? 'bg-blue-500/10 text-blue-600 border border-blue-500/20' : 'text-base-content/70 hover:bg-base-200 hover:text-base-content'}" on:click|preventDefault={() => handleMenuClick(subitem.path)}>
  ...
  <div class="border-t border-base-300 p-4 flex-shrink-0 mt-auto">
    <div class="flex items-center">
      <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=John" alt="User avatar" class="w-10 h-10 rounded-full object-cover bg-base-200" />
      ...
      <div class="flex items-center gap-2">
        <button class="p-1.5 rounded-md text-base-content/60 hover:text-red-600 hover:bg-red-100 transition-colors" on:click={handleLogout} title={$_("common.logout")}>
