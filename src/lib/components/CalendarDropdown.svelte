...
<div bind:this={dropdownElement} class="absolute right-0 mt-2 w-80 bg-base-100 rounded-lg shadow-lg border border-base-300 z-50">
  ...
  <div class="flex items-center justify-between p-4 border-b border-base-300">
    <h3 class="text-lg font-semibold text-base-content">Calendar</h3>
    <button class="p-1 text-base-content/60 hover:text-base-content hover:bg-base-200 rounded transition-colors focus:outline-none focus:ring-2 focus:ring-primary" on:click={onClose} aria-label="Close calendar menu">
  ...
  <!-- Calendar Actions -->
  <div class="py-2">
    {#each calendarActions as action (action.title)}
