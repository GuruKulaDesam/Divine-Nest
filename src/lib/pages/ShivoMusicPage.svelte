<script lang="ts">
  ...
  function startVoiceListening() {
    ...
    speechCaptureCleanup = () => stopSpeechCapture();
    startSpeechCapture({
      continuous: true,
      onResult: (text) => {
...
<div class="space-y-6">
  ...
  <div class="flex items-center justify-between">
    ...
    <div class="flex gap-3">
      {#if isListening}
        <button class="btn btn-error" on:click={stopVoiceListening}>
          <Icon icon="heroicons:microphone" class="w-5 h-5 mr-2" />
          Stop Listening
        </button>
      {:else}
        <button class="btn btn-primary" on:click={startVoiceListening}>
          <Icon icon="heroicons:microphone" class="w-5 h-5 mr-2" />
          Start Listening
        </button>
      {/if}
    ...
    <div class="card-body">
      ...
      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {#each musicTemplates as template}
          <button class="btn btn-outline btn-block h-auto py-4" on:click={() => generateFromTemplate(template.key)} disabled={isGenerating}>
    ...
    <div class="card-body">
      ...
      <div class="flex flex-wrap gap-4 items-center">
        ...
        <div class="flex items-center gap-3">
          ...
          <input type="range" min="0" max="1" step="0.1" bind:value={volume} on:input={() => setVolume(volume)} class="range range-primary" />
        ...
        <!-- Record Humming -->
        <button class="btn btn-secondary" on:click={startHummingRecording} disabled={isGenerating}>
          <Icon icon="heroicons:microphone" class="w-5 h-5 mr-2" />
          Record Melody
        </button>
      </div>
  ...
  <div class="card bg-base-100 shadow-lg">
    <div class="card-body">
      ...
      <div class="flex gap-3">
        ...
        />
        <button class="btn btn-primary" on:click={() => currentPrompt && generateMusicFromPrompt(currentPrompt)} disabled={isGenerating || !currentPrompt.trim()}>
  ...
  <div class="card bg-base-100 shadow-lg">
    <div class="card-body">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        {#each ["Shivo, make a Tamil folk melody with ambient pads", "Create a devotional tune with flute and tanpura", "Generate a chill beat with soft percussion and strings", "Classical orchestral piece with piano and strings", "Electronic ambient music with synthesizers", "World music fusion with traditional instruments"] as prompt}
          <button class="btn btn-ghost btn-block justify-start text-left h-auto py-3" on:click={() => generateMusicFromPrompt(prompt)} disabled={isGenerating}>
