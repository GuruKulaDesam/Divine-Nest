...
function SettingsPage($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<!----> <span>${escape_html(store_get($$store_subs ??= {}, "$_", $format)("settings.display"))}</span></div></button></nav></div></div> <div class="lg:col-span-3">`);
    {
      ...
      $$renderer2.push(`</div> <div><label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">${escape_html(store_get($$store_subs ??= {}, "$_", $format)("settings.language"))}</label> `);
      $$renderer2.select(
        {
