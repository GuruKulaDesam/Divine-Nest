...
function RecipesPage($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<!----> Add Recipe</button></div></div> <div class="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-lg p-6 shadow-sm"><div class="flex items-center justify-between"><div class="flex-1"><h2 class="text-xl font-bold text-amber-800 mb-2">🍲 பாட்டியின் சமையல் ஞானம்</h2> <p class="text-amber-700 mb-4">Discover 25 traditional Tamil recipes from ancient family wisdom - medicine for body, peace for mind, love for family</p> <div class="flex flex-wrap gap-2 text-sm"><span class="bg-amber-100 text-amber-800 px-3 py-1 rounded-full">Daily Sattvic Foods</span> <span class="bg-amber-100 text-amber-800 px-3 py-1 rounded-full">Fasting Foods</span> <span class="bg-amber-100 text-amber-800 px-3 py-1 rounded-full">Festival Foods</span> <span class="bg-amber-100 text-amber-800 px-3 py-1 rounded-full">Medicinal Foods</span> <span class="bg-amber-100 text-amber-800 px-3 py-1 rounded-full">Family Love Foods</span></div></div> <div class="ml-6"><a${attr("href", `${stringify(base)}/recipes/traditional`)} class="btn btn-warning gap-2 shadow-lg hover:shadow-xl transition-all duration-300">`);
    Icon($$renderer2, { icon: "heroicons:sparkles", class: "w-5 h-5" });
    ...
    $$renderer2.push(`<!----></div> <div class="stat-title">Active Recipes</div> <div class="stat-value text-info">${escape_html(recipes.filter((r) => r.status === "Active").length)}</div></div></div> <div class="bg-base-100 p-6 rounded-lg shadow"><div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4"><div class="form-control"><label for="recipe-search" class="label"><span class="label-text">Search Recipes</span></label> <input id="recipe-search" type="text"${attr("value", searchQuery)} placeholder="Search by name..." class="input input-bordered"/></div> <div class="form-control"><label for="recipe-category" class="label"><span class="label-text">Category</span></label> `);
    $$renderer2.select(
      {
        id: "recipe-category",
    ...
    $$renderer2.push(`</div> <div class="form-control"><label for="recipe-cuisine" class="label"><span class="label-text">Cuisine</span></label> `);
    $$renderer2.select(
      {
        id: "recipe-cuisine",
    ...
    $$renderer2.push(`</div> <div class="form-control"><label for="recipe-difficulty" class="label"><span class="label-text">Difficulty</span></label> `);
    $$renderer2.select(
      {
    ...
    $$renderer2.push(`</div> <div class="form-control"><button class="btn btn-outline btn-block">Clear Filters</button></div></div></div> <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"><!--[-->`);
    ...
    for (let $$index_3 = 0, $$length = each_array_3.length; $$index_3 < $$length; $$index_3++) {
      ...
      $$renderer2.push(`<!----></button> <ul class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52"><li><button class="btn btn-ghost">Edit</button></li> <li><button class="btn btn-ghost text-error">Delete</button></li></ul></div></div> <div class="flex flex-wrap gap-2 mb-3"><span class="badge badge-primary">${escape_html(recipe.category)}</span> <span class="badge badge-secondary">${escape_html(recipe.cuisine)}</span> <span${attr_class(`badge ${stringify(getDifficultyColor(recipe.difficulty))}`)}>${escape_html(recipe.difficulty)}</span> <span${attr_class(`badge ${stringify(getStatusColor(recipe.status))}`)}>${escape_html(recipe.status)}</span></div> <div class="grid grid-cols-2 gap-4 text-sm mb-4"><div class="flex items-center gap-1">`);
      Icon($$renderer2, {
